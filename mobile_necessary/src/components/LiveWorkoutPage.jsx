...
export default function LiveWorkoutPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#12080d', color: 'white', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Main */}
      <main style={{ flex: 1, padding: 'clamp(1rem, 4vw, 2.5rem)', maxWidth: '1280px', margin: '0 auto', width: '100%', position: 'relative', zIndex: 10 }}>
        {/* Header */}
        <header style={{ marginBottom: '2rem' }}>
          <h1 className="heading" style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', letterSpacing: '-0.04em', textTransform: 'uppercase', lineHeight: 1, marginBottom: '1.25rem' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', width: '100%', sm: { width: 'auto' } }}>
              <CtaButton id="btn-start-camera" icon="lucide:camera" label={sessionActive ? 'Active' : 'Start'} primary onClick={sessionActive ? undefined : startSession} />
              <CtaButton id="btn-stop" icon="lucide:square" label="Stop" variant="danger" onClick={stopSession} />
              <CtaButton id="btn-ai-feedback" icon="lucide:brain" label={aiLoading ? "..." : "AI"} variant="emerald" onClick={() => maybeRequestAiFeedback(reps, exercise.toLowerCase().replace('-', ''), true)} />
              <CtaButton id="btn-voice-coach" icon="lucide:mic" label={voiceListening ? "..." : "Coach"} variant="cyan" onClick={startVoiceInput} />
            </div>
          </div>
        </header>

        {/* Grid */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem' }} className="live-grid">
          {/* Camera feed card */}
          <div
            ref={cardRef}
            style={{
          ...
          >
            ...
            <div style={{ position: 'absolute', top: 'clamp(1rem, 5vw, 2rem)', left: 'clamp(1rem, 5vw, 2.5rem)', display: 'flex', flexDirection: 'column', gap: '0.75rem', zIndex: 10 }}>
              <div style={{
                ...
                <span style={{ fontSize: '0.5rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#d4a574', display: 'block', marginBottom: '0.2rem' }}>
                  Reps
                </span>
                <span
                  className="heading"
              ...
              <div style={{
                ...
                <span style={{ fontSize: '0.5rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#22d3ee', display: 'block', marginBottom: '0.25rem' }}>
                  Posture
                </span>
                <p style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.85)', lineHeight: 1.4, fontWeight: 500 }}>
                  {postureTip}
                </p>
              </div>
            </div>
            ...
            {/* Live badge */}
            <div style={{
              position: 'absolute', bottom: 'clamp(1rem, 5vw, 2rem)', right: 'clamp(1rem, 5vw, 2.5rem)', zIndex: 10,
              ...
              {sessionActive && <div style={{ width: '6px', height: '6px', background: 'white', borderRadius: '50%' }} className="animate-pulse" />}
              {sessionActive ? 'Live' : 'Camera Off'}
            </div>
          </div>
          ...
          <div style={{
            ...
            height: 'clamp(320px, 50vh, 480px)', // Strict finite boundary
            position: 'relative',
            overflow: 'hidden',
          }}>
