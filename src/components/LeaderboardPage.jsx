  ...
  return (
    <div style={{ minHeight: '100vh', background: '#12080d', color: 'white', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <main style={{ flex: 1, padding: '2.5rem', maxWidth: '1280px', margin: '0 auto', width: '100%', position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
        ...
        {/* ── Podium (top 3) ── */}
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem', alignItems: 'flex-end', marginBottom: '1rem' }} className="podium-grid">
          {currentPodium.map((p) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2.5rem' }} className="lb-main-grid">
          ...
          <div style={{ background: 'rgba(42,18,32,0.2)', borderRadius: '2.5rem', border: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden', backdropFilter: 'blur(10px)' }}>
            {/* Table header */}
            <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 160px 120px', padding: '1rem 2rem', background: 'rgba(42,18,32,0.6)', borderBottom: '1px solid rgba(255,255,255,0.08)' }} className="table-header-grid">
              {['Rank', 'Elite User', 'Tier Status', 'Total Reps'].map((h, i) => (
                <span key={h} style={{ fontSize: '0.55rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.2em', color: 'rgba(255,255,255,0.35)', textAlign: i === 3 ? 'right' : 'left' }}>{h}</span>
              ))}
            </div>
    ...
    <div style={{ order, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
      ...
      >
        ...
        <div style={{
          ...
          width: p.champion ? '112px' : '88px',
          height: p.champion ? '112px' : '88px',
          borderRadius: '50%',
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <h3 className="heading" style={{ fontSize: p.champion ? '1.5rem' : '1.25rem', textTransform: 'uppercase', letterSpacing: '-0.03em' }}>{p.name}</h3>
        </div>
        ...
        {/* Rep count */}
        <span className="heading" style={{ fontSize: p.champion ? '2.5rem' : '2rem', color: p.champion ? 'white' : '#d4a574' }}>
          {p.reps} <span style={{ fontSize: '0.7em', color: 'rgba(255,255,255,0.4)', fontFamily: 'Satoshi, sans-serif', fontWeight: 700 }}>Reps</span>
        </span>
      </div>
...
function TableRow({ row }) {
  ...
  return (
    ...
    >
      ...
      {/* Tier */}
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', background: row.isYou ? 'rgba(212,165,116,0.15)' : 'rgba(255,255,255,0.04)', padding: '0.3rem 0.85rem', borderRadius: '0.5rem', border: `1px solid ${row.isYou ? 'rgba(212,165,116,0.35)' : 'rgba(255,255,255,0.08)'}`, width: 'fit-content' }}>
